...
/*HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
    ...
    g_window.m_hWnd = CreateWindow( "TutorialWindowClass", "Direct3D 11 Tutorial 7", WS_OVERLAPPEDWINDOW,
...
HRESULT InitDevice()
{
    ...
    hr = CompileShaderFromFile( "YoRHaEngine.fx", "VS", "vs_4_0", &pVSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( NULL,
    ...
    hr = CompileShaderFromFile("YoRHaEngine.fx", "PS", "ps_4_0", &pPSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( NULL,
                    "The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", "Error", MB_OK );
