...
Texture::init(Device device, std::string textureName)
{
	...
	{
		ERROR("Texture", "init", "CHECK FOR Device on texture loading method")
			exit(1);
	}
	...
	if (FAILED(hr))
	{
		ERROR("Texture", "init", "Error in data from params [CHECK FOR string textureName -> Verifyy correct texture name in Filepath]");
		exit(1);
	}
	return hr;
}
	               ...
	               unsigned int BindFlags)
{
	if (device.m_device == nullptr)
	{
		ERROR("Texture", "init", "CHECL FOR Device on texture loading method")
			exit(1);
	}
	else if(width <= 0 ||height <= 0)
	{
		ERROR("Texture", "init", "CHECL FOR unsigned int width or unsigned int height")
			exit(1);
	}
	HRESULT hr = S_OK;
